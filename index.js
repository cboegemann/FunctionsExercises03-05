...
function reverseCharactersPart2(x) {
    if (typeof x === "number") {
      x = String(x);
 }  let array = x.split("").reverse().join("");
    return array;
...
function reverseArray(oldArray) {
    ...
    for (i = 0; i < oldArray.length; i++) {
        newArray.push(reverseCharactersPart2(oldArray[i]));
    }
    return newArray;
}
...
console.log(arrayToChange(arrayTest2));
console.log(arrayToChange(arrayTest3));